/*  wefax.c
 *
 *  WEFAX decoding functions for xwefax application
 */
  ...
  switch( wefax_action )
  {
	case ACTION_BEGIN: /* Begin decoding process */
	  ...
	  scrollwin = GTK_SCROLLED_WINDOW(
		  lookup_widget(main_window, "image_scrolledwindow") );
...
Wefax_Decode( void )
{
  ...
  if( first_call )
  {
	...
	gdk_window_invalidate_rect(
		gbl_wefax_drawingarea->window, NULL, FALSE );
  ...
  if( isFlagSet(RECEIVE_STOP) && gbl_line_count )
  {
	...
	if( isFlagSet(SAVE_IMAGE_PGM) && isFlagSet(SAVE_IMAGE) )
	{
	  ...
	  if( !Open_File(&fp, file_name_pgm, "w") ||
		  !Save_Image_pgm(fp, "P5", rc_data.pixels_per_line,
			gbl_line_count, 255, image_buffer) )
	  {
	...
	if( isFlagSet(SAVE_IMAGE_JPG) && isFlagSet(SAVE_IMAGE) )
	{
	  ...
	  if( !Open_File(&fp, file_name_jpg, "w") ||
		  !Save_Image_jpg(fp, rc_data.pixels_per_line,
			gbl_line_count, image_buffer) )
	  {
  ...
  gbl_linebuff_input++;
  if( gbl_linebuff_input >= rc_data.line_buffer_size )
	gbl_linebuff_input = 0;
  ...
  discr_max_idx = 0;
  discr_op_max  = -256;
  image_buffer_idx = gbl_line_count * rc_data.pixels_per_line;
  for( pixel_idx = 0; pixel_idx < rc_data.pixels_per_line; pixel_idx++ )
  {
		...
		(pixel_idx < INIMAGE_PHASING_RANGE) )
	{
	  ...
	  discr_op_ave -= (double)gbl_line_buffer[ gbl_linebuff_output ];
	...
	{
	  if( gbl_line_buffer[ gbl_linebuff_output ] > BILEVEL_THRESHOLD )
		gbl_line_buffer[ gbl_linebuff_output ] = 255;
	  else
		gbl_line_buffer[ gbl_linebuff_output ] = 0;
	}

	/* Copy line buffer to currrent image buffer line */
	image_buffer[ image_buffer_idx + pixel_idx ] =
	  gbl_line_buffer[ gbl_linebuff_output ];
	gbl_linebuff_output++;
	if( gbl_linebuff_output >= rc_data.line_buffer_size )
	  gbl_linebuff_output = 0;
	  ...
	  (discr_op_max > INIMAGE_SYNC_THRESHOLD) )
  {
	...
	if( gbl_linebuff_input >= rc_data.line_buffer_size )
	  gbl_linebuff_input -= rc_data.line_buffer_size;
  ...
  for( pixel_idx = 0; pixel_idx < rc_data.pixels_per_line; pixel_idx++ )
  {
	pixel =
	  gbl_pixel_buf +
	  gbl_n_channels * pixel_idx +
	  gbl_line_count * gbl_rowstride;
  ...
  rect.x = 0;
  rect.y = gbl_line_count;
  rect.width = rc_data.pixels_per_line;
  rect.height = 1;
  gdk_window_invalidate_rect(
	  gbl_wefax_drawingarea->window, &rect, FALSE );
  ...
  int diff = gbl_linebuff_input - gbl_linebuff_output;
  if( (diff < 0) && (diff >= -rc_data.pixels_per_line) )
	gbl_linebuff_input += rc_data.pixels_per_line;
  ...
  gbl_line_count++;
  if( gbl_line_count >= rc_data.image_lines )
  {
  ...
  if( stop && gbl_line_count )
  {
	...
	wefax_action = ACTION_BEGIN;
	return( TRUE );
  } /* if( stop && gbl_line_count && isFlagSet(SAVE_IMAGE) ) */

  /* Re-allocate image buffer per line */
  buf_size = (size_t)( (gbl_line_count + 1) * rc_data.pixels_per_line );
...
Wefax_Drawingarea_Button_Press( GdkEventButton  *event )
{
  ...
  if( event->button == 1 )
  {
	gbl_linebuff_input -= (int)(event->x + 0.5);
	if( gbl_linebuff_input < 0 )
	  gbl_linebuff_input += rc_data.line_buffer_size;
 }
...
{
  GtkLabel *lbl = GTK_LABEL( lookup_widget(main_window, "rcve_status") );
  if( gtk_toggle_button_get_active(togglebutton) )
  {
	...
	gbl_linebuff_output =
	  rc_data.line_buffer_size - rc_data.pixels_per_line2;
	wefax_action = ACTION_BEGIN;
	gtk_idle_add( Wefax_Control, NULL );
  }
  else
  {
